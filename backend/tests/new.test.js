...
describe("Noticias API", () => {
  ...
  it("GET /noticias - debe devolver un array de noticias", async () => {
    NewsRepository.getAll.mockImplementation((cb) =>
      cb(null, [{ id: 1, titulo: "Test" }])
    );
  ...
  it("POST /noticias - crear noticia", async () => {
    ...
    const res = await request(app).post("/noticias").send({
      titulo: "Nueva",
      contenido: "Contenido",
      autor: "Juan",
      imagen: "img.png",
  ...
  it("GET /noticias/:id - obtener por id", async () => {
    NewsRepository.getById.mockImplementation((id, cb) =>
      cb(null, { id, titulo: "Test" })
    );
  ...
  it("PUT /noticias/:id - actualizar noticia", async () => {
    ...
    const res = await request(app).put("/noticias/1").send({
      titulo: "Edit",
      contenido: "Contenido",
      autor: "Juan",
      imagen: "img.png",
